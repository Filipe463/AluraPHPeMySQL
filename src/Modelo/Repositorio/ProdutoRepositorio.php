...
class ProdutoRepositorio
{
    ...
    {
        $slq = "SELECT * FROM produtos ORDER BY preco";
        $statement = $this->pdo->query($slq);
    ...
    public function deletar(int $id)
    {
        ...
        $statemente = $this->pdo->prepare($sql);
    ...
    public function buscar(int $id)
    {
        ...
        $daods = $statement->fetch(PDO::FETCH_ASSOC);
        return $this->formarObjeto($daods); 
    }
    ...
    public  function atualizar(Produto $produto)
    {
        ...
        if($produto->getImagem() !== 'logo-serenatto.png')
            {
                $this->atualizaFoto($produto);
            }      

    }
    private function atualizaFoto(Produto $produto)
    {
        $sql = 'UPDATE produtos SET imagem = ? where id = ?';
        $statemente = $this->pdo->prepare($sql);
        $statemente->bindValue(1,$produto->getImagem());
        $statemente->bindValue(2,$produto->getId());
        $statemente->execute();
    }
} 
?>